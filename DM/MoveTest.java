...
private static final double GUN_TURNING_RATE = 40.0 / 180.0 * Math.PI;  // This is a guess... (radians per tick)
private static final double START_HIT_TIME = 5;	
private static final double TOL = 0.5;				// tolerance; how close we must aim before we decide to fire
private static final double MAX_ITERATIONS = 10;	// how many times we try for an aiming solution per turn
private static final double BULLET_POWER = 1.5;		// Strength of our bullets
...
boolean guessed = false;
	...
	void ParGun()
		...
		if(enemy.numGunScans%12==0||enemy.numGunScans==0){
	...
	//
	//
	if(/*enemy.speed > 1*/false){
	double bulletSpeed = 20 - 3 * bPower;
		// bulletSpeed is re-calculated every time! This is unnecessary but allows
		// us to modify in order to make the bullet power vary if we want.
		
		double de = e.getDistance();									// Distance to enemy 
		double be = e.getBearingRadians();								// Angle to enemy
		double ed = e.getHeadingRadians() - this.getHeadingRadians(); 	// Enemy's direction
		double ev = e.getVelocity();									// Enemy speed
		
		double ett = amountToTurn(be) / GUN_TURNING_RATE;				// Estimated time to turn
		...
		do {
			...
			Point enemy = new Point(de * Math.sin(be), de * Math.cos(be));  // Enemy location

			double enemyx = enemy.x;
			double enemyy = enemy.y;

			// Take enemy movement into account
			enemy.x += hit_time * ev * Math.sin(ed);
			enemy.y += hit_time * ev * Math.cos(ed);
			
			// Take our movement into account
			enemy.y -= ett * this.getVelocity();
			
			/*if(enemyx < 0)
				enemyx = 0;
			if(enemyy < 0)
				enemyy = 0;
			if(enemyx > getBattleFieldWidth())
				enemyx = getBattleFieldWidth();
			if(enemyy > getBattleFieldHeight())
				enemyy = getBattleFieldHeight();*/
			
			double fire_length = enemy.distanceFromOrigin();
			double fire_time = fire_length / bulletSpeed;
			fire_angle = Math.atan(enemy.x / enemy.y);
			if (enemy.y < 0)
				fire_angle += Math.PI;
		...
		//out.println("Firing... ETA " + actual_time + " ticks");
		turnGunRightRadians(amountToTurn(fire_angle));

		//out.println(fire_angle);
		//fire(bPower);
	}
	else{
		//turnGunRight(bearingFromGun);
